...
@section('content')
    ...
    @if(!empty($reportData['weeks']) && $reportType != 'summary')
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
        <!-- Gráfico de Barras -->
        <div class="bg-white rounded-lg shadow-sm p-6">
            <h3 class="text-lg font-medium text-gray-900 mb-4">Consumo por Período</h3>
            <canvas id="barChart" height="200"></canvas>
        </div>

        <!-- Gráfico de Pizza -->
        <div class="bg-white rounded-lg shadow-sm p-6">
            <h3 class="text-lg font-medium text-gray-900 mb-4">Distribuição de Consumo</h3>
...
document.addEventListener('DOMContentLoaded', function() {
    const data = @json($reportData['weeks']);
    ...
    const consumption = data.map(item => item.feed_consumption_kg);
    const colors = [
        'rgba(59, 130, 246, 0.8)',
        'rgba(16, 185, 129, 0.8)',
        'rgba(245, 158, 11, 0.8)',
